...
void nanolog_benchmark()
{
  ...
  uint64_t begin = bench_timestamp_now();
  for (int i = 0; i < iterations; ++i)
    LOG_INFO << "Logging " << benchmark << i << 0 << 'K' << -42.42;
  uint64_t end = bench_timestamp_now();
  long int avg_latency = (long int)(end - begin) * 1000 / iterations;
...
int main_bench()
{
    ...
    nanolog::initialize(nanolog::NonGuaranteedLogger(10), "c://tmp//", "log_bench", 1);
